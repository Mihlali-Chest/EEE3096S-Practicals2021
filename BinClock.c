...
void pin18_isr_callback(void){
        hourInc();
}
void pin27_isr_callback(void){
        minInc();
}
...
void initGPIO(void){
        ...
        wiringPiISR(5, INT_EDGE_BOTH, pin18_isr_callback);
        wiringPiISR(30, INT_EDGE_BOTH, pin27_isr_callback);
...
void minInc(void){
        ...
        if (interruptTime - lastInterruptTime>200){
                ...
                if (mins == 59){
                               mins = 0;
                               mins = wiringPiI2CWriteReg8(RTC, MIN_REGISTER, mins);
...
void controlTime(void){
                if (secs == 59){
                                secs = 0;
                                secs = wiringPiI2CWriteReg8(RTC, SEC_REGISTER, secs);
                                mins+=1;
                                mins =  wiringPiI2CWriteReg8(RTC, MIN_REGISTER, mins);
                }
                else if (mins == 59){
                           ...
                           mins =  wiringPiI2CWriteReg8(RTC, MIN_REGISTER, mins);
                           hours+=1;
                           hours =  wiringPiI2CWriteReg8(RTC, HOUR_REGISTER, hours);
                }
                else if(hours == 23){
